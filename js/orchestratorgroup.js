...
function discoverAndBuildUI(node) {
    ...
    const handleRadioClick = (clickedWidget) => {
        ...
        for (const widget of node.widgets) {
            if (widget.type === 'toggle') {
                ...
                if (group) {
                    const newMode = widget.value ? 0 : 4;
    ...
    for (const group of app.graph._groups) {
        if (!existingWidgets.has(group.title)) {
            ...
            const anyOtherIsEnabled = node.widgets.some(w => w.type === 'toggle' && w.value === true);
            if (anyOtherIsEnabled || (group._nodes.length > 0 && group._nodes[0].mode === 4)) {
    ...
    if (hasChanges || (!toggleExists && originalWidgetCount > 0)) {
        ...
        const currentWidth = node.size[0]; // On mémorise la largeur actuelle
        const newComputedSize = node.computeSize(); // On calcule la nouvelle taille nécessaire
        node.size = [currentWidth, newComputedSize[1]]; // On applique la nouvelle hauteur mais on garde l'ancienne largeur
...
app.registerExtension({
	name: "Comfy.GroupBypasser.Heartbeat.Radio.Spacer.FixedResize",

	beforeRegisterNodeDef(nodeType, nodeData, app) {
		if (nodeData.name === "OrchestratorNodeGroupBypasser") {
